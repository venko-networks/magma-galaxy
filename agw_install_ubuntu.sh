...
addr1=$(ip -br a | grep -v 127.0 | awk 'FNR==1{print $3}')
gw_addr=$(ip r s default | cut -d' ' -f3)
...
echo "Installing mokutil to verify SecureBoot"
apt install mokutil
echo "Checking if SecureBoot is disabled"
...
if [ "$MAGMA_INSTALLED" != "$SUCCESS_MESSAGE" ]; then
  ...
  TASK_FILE="/home/$MAGMA_USER/magma/lte/gateway/deploy/roles/magma_deploy/tasks/main.yml"
  # Comment out the block for unvalidated apt signing key
  sed -i '/- name: Add unvalidated Apt signing key/,/^$/ s/^/#/' "$TASK_FILE"
